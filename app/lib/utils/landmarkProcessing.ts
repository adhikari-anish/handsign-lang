...
import { HandLandmark, PredictionResult } from "../types/webcam.types";
import { ALPHABET, CONFIDENCE_THRESHOLD } from "../constants/webcam.constants";

export const preprocessHandLandmarks = (
  handWorldLandmarks: HandLandmark[][],
): tf.Tensor => {
  if (handWorldLandmarks.length < 1) {
    return tf.tensor([], [0, 0, 0], "float32");
  }

  const coordinates: number[][] = handWorldLandmarks.flatMap
  ((handLandmarks) => 
